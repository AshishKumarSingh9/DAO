//* This creates a new proposal to withdraw all the funds from our 'Treasury' contract.
...
export async function propose(args: any[], functionToCall: string, proposalDescription: string) {
  ...
  const treasury = await ethers.getContract("Treasury"); // need Treasury contract, as we are proposing changes to Treasury contract.
  ...
  const encodedFunctionCall = treasury.interface.encodeFunctionData(
    functionToCall,
    args
  ); //creating 'encoded' parameters for functions that we want to call on Treasury contract. This will turn it into bytes calldata. It has two arguments: 'functionToCall' and 'args', so we will include them in our propose function's argument.

  // console.log(encodedFunctionCall);

  console.log(`Proposing ${functionToCall} on ${treasury.address} to ${args}`);
...
propose([PAYEE_ADDRESS], FUNC, PROPOSAL_DESCRIPTION)
  .then(() => process.exit(0))
  .catch((error) => {
    console.log(error);
    process.exit(1);
